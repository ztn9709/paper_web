<template>
  <div :class="{ ptable: true, 'ptable-full': full }">
    <div class="ptable-main">
      <div
        :class="{ 'ptable-row': true, 'ptable-row-full': full }"
        v-for="i in 7"
      >
        <div :class="classes(i, j)" v-for="j in 18" @click="clickCell(i, j)">
          <i
            :class="{ 'element-id': true, 'element-id-full': full }"
            @mousedown.stop.prevent
            >{{ index(i, j) }}</i
          >
          <strong
            :class="{ 'element-symbol': true, 'element-symbol-full': full }"
            @mousedown.stop.prevent
            >{{ symbol(i, j) }}</strong
          >
        </div>
...
export default {
  ...
  methods: {
    handleClick(elem) {
      ...
      $emit(this, 'update:value', this.elems)
    },
