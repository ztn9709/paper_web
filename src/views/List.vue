<template>
  <div id="box">
    <el-input placeholder="请输入关键词或作者或DOI号" v-model:value="text" clearable>
      <template v-slot:append>
        <el-button icon="el-icon-search" @click="handleCurrentChange(1)">搜索</el-button>
      </template>
    </el-input>
    <br />
    <br />
    <div class="block">
      ...
      <el-date-picker v-model:value="date" type="daterange" align="right" unlink-panels range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions" value-format="yyyy-MM-dd"> </el-date-picker>
    </div>
    <br />
    <div>选择期刊范围</div>
    <br />
    ...
    <el-card v-for="paper in paperList" :key="paper.DOI" class="box-card">
      <template v-slot:header>
        <div class="clearfix">
          ...
          <span
            ><el-link :href="paper.link" :underline="false" target="_blank" style="color: #3294d8; font-size: 30px">{{ paper.title }}</el-link></span
          >
    ...
    <br />
    <el-pagination @current-change="handleCurrentChange" v-model:current-page="currentPage" :page-size="pageSize" layout="total, prev, pager, next,jumper" :total="total"></el-pagination>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      gridData: [
        {
          title: 'Effect of magnetic impurity scattering on transport in topological insulators',
          similarity: 1
        },
        {
          title: 'Edge spin transport in the disordered two-dimensional topological insulator WTe2',
        ...
        {
          title: 'Elastic backscattering of quantum spin Hall edge modes from Coulomb interactions with nonmagnetic impurities',
        ...
        {
          title: 'Dynamic impurities in two-dimensional topological-insulator edge states',
          similarity: 0.9150208830833435
        },
        {
          title: 'Conductance suppression by nonmagnetic point defects in helical edge channels of two-dimensional topological insulators',
          similarity: 0.9142051935195923
        },
        {
          title: 'Disorder-induced currents as signatures of chiral superconductivity',
          similarity: 0.9139000773429871
        },
        {
          title: 'Two-spin entanglement induced by scattering of backscattering-free chiral electrons in a Chern insulator',
          similarity: 0.9130393862724304
        },
        {
          title: 'Backscattering off a driven Rashba impurity at the helical edge',
